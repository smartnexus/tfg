...
# Change if deploying on another instance.
#REACT_APP_API_URI="https://api.orchestramanager.pw" # Production
REACT_APP_API_URI="${REACT_APP_API_URI:="http://localhost:5000"}" # Development

echo "[job.build] Image output will be: $IMAGE_NAME - env: \$REACT_APP_API_URI=$REACT_APP_API_URI"
...
docker build --build-arg api_uri=$REACT_APP_API_URI -t $IMAGE_NAME $PROJECT_DIR/dist/$NAME
if [ $? -ne 0 ]; then
    echo " an error ocurred. Check build.log for details!"
    exit 1;
fi
